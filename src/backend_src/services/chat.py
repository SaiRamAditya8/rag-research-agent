...
from src.agents_src.pipeline.qa import QAPipeline
from src.agents_src.pipeline.chitchat import ChitChatPipeline
...
# RAGQueryTool (ChromaDB + LlamaIndex) and LLMClient (Groq SDK) are shared
# across all requests.
# ---------------------------------------------------------------------------
_intent_pipeline = IntentPipeline()
_qa_pipeline = QAPipeline()
_chitchat_pipeline = ChitChatPipeline()
...
def get_answer(user_query: str, session_id: str = "default_session") -> dict:
    ...
    # 5. Route to QA or ChitChat pipeline
    if intent.use_rag:
        logger.info("Routing to QA pipeline.")
        answer_obj = _qa_pipeline.run(
        ...
        logger.info("Routing to ChitChat pipeline.")
        answer_obj = _chitchat_pipeline.run(
            user_query=user_query,
            chat_history=chat_buffer,
            chat_summary=chat_summary,
            fetch=fetch_occurred,
